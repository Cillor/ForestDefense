...
public class PlayerController : MonoBehaviour
{
    ...
    private void RotateView()
    {
        ...
        xRot -= Input.GetAxis("Mouse Y") * YSensitivity * staminaReduceEffect.Evaluate(SaveManager.Instance.state.stamina) / 100;
        xRot = Mathf.Clamp(xRot, MinimumX, MaximumX);

        m_CharacterTargetRot *= Quaternion.Euler(0f, yRot, 0f);
       
        transform.localRotation = m_CharacterTargetRot;
        m_Camera.transform.localEulerAngles = new Vector3(xRot, 0, 0);
    }
